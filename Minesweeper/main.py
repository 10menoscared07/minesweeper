...
class Cell:
    ...
    def draw(self, window):
        ...
        if self.revealAnimating and self.revealCoverShow:
            ...
            if self.textImage:
                window.blit(self.textImage, self.textRect)
            pygame.draw.rect(window, self.revealColor, self.revealRect)


        if self.isHovered:
            pygame.draw.rect(window, self.hoverColor, self.rect, 7)

        if self.flagged:
            pygame.draw.rect(window, self.flagColor, self.rect, 5)
            pygame.draw.line(window, self.flagColor, self.rect.topleft, self.rect.bottomright, 5)
            pygame.draw.line(window, self.flagColor, self.rect.topright, self.rect.bottomleft, 5)
...
class Minesweeper:
    def __init__(self, res):
        ...
        self.restartText = self.font.render("Press [R] to restart", True, self.colors[2], None)
        self.restartTextBase = self.font.render("Press [R] to restart", True, self.colors[2], None)
        ...
        for y in range(int(self.numCells.y)):
            for x in range(int(self.numCells.x)):
                ...
                if numMines != 0:
                    ...
                    cell.setValue(numMines, font.render(" ", True, (200,200,200), None))
